...
from helper_func import MyResNet18,GetResNet101,EnsamblePred,EnsamblePredForAUC,GetNumOfUniqValues_torch_ver,ConvertTargetsToOrdinal,Myfloat_oupt_to_class
...
tmp_targets_val = torch.reshape(val_labels,(-1,1)).float().cuda()
val_weights = torch.reshape(val_weights,(-1,1)).float().cuda()
y_true = tmp_targets_val.cpu().detach().numpy()
y_true_bin = y_true>0.9
targets_val = ConvertTargetsToOrdinal(tmp_targets_val,K=4)
...
for Epoch in range(EPOCHS):
    for batch_i, [batch,labels,weights] in enumerate(train_dataloader):
        ...
        targets = ConvertTargetsToOrdinal(targets,K=4)
        loss = torch.mean(loss_fn(outputs,targets)*weights)*4
        ...
        with torch.no_grad():
            ...
            prob_preds = np.clip(outputs_val.cpu().numpy(),0.0,0.25)*4 # Not exactly good predictor..
            AUC_val = roc_auc_score(y_true_bin, prob_preds)                         
           
            loss_val_weighted = torch.mean(loss_fn_val(outputs_val,targets_val)*val_weights)*4 #should be AUC for bin case, and L1 (as now) for multy label
            loss_val = torch.mean(loss_fn_val(outputs_val,targets_val))*4
...
with torch.no_grad():    
    ...
    FixedProb = np.copy(prob_preds)
    FixedProb[np.where(NumOfUniqValues > 19)[0],0] = 0
    Fixed_AUC_val = roc_auc_score(y_true_bin,  np.clip(np.sum(FixedProb[:,1:],axis=1),0.0,1.0))
    print('AUC on val set with UniqValues fix:',Fixed_AUC_val)

### Track 2 Eval
with torch.no_grad(): 
    predicted_method_1 = Myfloat_oupt_to_class(outputs_val, K=4).reshape(-1,)
    correct_method_1 = (predicted_method_1 == tmp_targets_val.reshape(-1,)).sum().item()
